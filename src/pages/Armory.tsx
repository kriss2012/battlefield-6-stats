...
const Armory: React.FC = () => {
  ...
  const [accessory, setAccessory] = useState<string | undefined>(undefined);

  const operators = [
    { id: 1, name: 'VANGUARD', role: 'Assault', desc: 'Frontline combat specialist with adaptive armor.', color: '#3b82f6' },
    { id: 2, name: 'GHOST', role: 'Infiltrator', desc: 'Stealth-focused operative with thermal suppression.', color: '#10b981' },
    { id: 3, name: 'TITAN', role: 'Engineer', desc: 'Heavy support unit capable of field repairs.', color: '#f59e0b' },
  ];

  const weapons = [
    { id: 1, name: 'M5A3 MKII', type: 'Assault Rifle', desc: 'Standard issue modular weapon platform.', color: '#4b5563' },
    { id: 2, name: 'SWS-10', type: 'Sniper', desc: 'Long-range precision kinetic energy rifle.', color: '#1e293b' },
    { id: 3, name: 'VECTOR-9', type: 'SMG', desc: 'High-rate spectral combat submachine gun.', color: '#6366f1' },
  ];
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-8">
          <div>
            ...
            <p className="text-gray-500 font-medium max-w-xl">
              Customize your combat readiness with high-fidelity 3D previews and material synthesis.
            </p>
          </div>

          <div className="flex gap-1 p-1 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-md">
            <button 
              onClick={() => { setActiveTab('operators'); setSelectedId(1); setAccessory(undefined); }}
            ...
            <button 
              onClick={() => { setActiveTab('arsenal'); setSelectedId(1); setAccessory(undefined); }}
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="glass-card p-8">
              ...
              {activeTab === 'arsenal' && (
                <div className="space-y-4 mb-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      { id: 'none', label: 'BARE' },
                      { id: 'scope', label: '6X OPTIC' },
                      { id: 'suppressor', label: 'SILENCER' },
                    ].map((att) => (
                      <button
                        key={att.id}
                        onClick={() => setAccessory(att.id === 'none' ? undefined : att.id)}
                        className={`px-4 py-2 border rounded-xl text-[10px] font-black italic uppercase transition-all ${
                          (att.id === 'none' && !accessory) || accessory === att.id 
                ...
                <div className="flex gap-3">
                  {['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ffffff'].map((color) => (
                    <button
                      ...
                      className={`w-10 h-10 rounded-xl border-2 transition-all ${customColor === color ? 'border-white scale-110' : 'border-transparent hover:scale-105'}`}
              ...
              <button className="w-full py-4 bg-white text-black rounded-2xl font-black italic uppercase tracking-widest hover:bg-blue-400 transition-colors flex items-center justify-center gap-3">
              ...
              {currentList.map((item) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <h4 className="font-black italic text-sm uppercase tracking-wide mb-0.5">{item.name}</h4>
